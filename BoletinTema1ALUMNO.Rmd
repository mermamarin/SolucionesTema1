...
```{r}
intersect<-intersect(Animals2,mammals)
rownames(intersect)
commonAnimals<-rownames(intersect)
commonAnimals
...
# Son exactamente 3 animales, caracterizados por ser especies de dinosaurios
...
```{r}
# La diferencia entre ellos son los valores de las dos variables categóricas que forman parte de las columnas del data, que son body y brain.
...
# Si que presentan el mismo comportamiento y dado que las muestras están dentro de las líneas discontinuas, podemos indicar que siguen una distribución Gaussiana en ambos casos.
...
```{r}
libReturn<-library()
head(libReturn$results[,1], n = 5)
libReturn$results[1:5,1]
...
#Sí, los datos siguen una distribución Gaussiana
```
...
```{r}
ggplot(cabbages, aes (x = VitC, color = Cult)) + geom_histogram()
cabbages$Cult
ggplot(cabbages, aes (x = VitC, fill = Cult)) + geom_histogram(alpha = 0.5)

hist(cabbages$VitC[cabbages$Cult =='c39'], main = paste("Variedad c39"))
hist(cabbages$VitC[cabbages$Cult =='c52'], main = paste("Variedad c52"))
...
# Planteamos 2 modelos lineales, uno para los datos de cada factor
Model11 <- lm(VitC ~ HeadWt, data = cabbages, subset = which(Cult == "c39"))
Model22 <- lm(VitC ~ HeadWt, data = cabbages, subset = which(Cult == "c52"))

# Representamos las rectas correspondientes a cada modelo lineal
abline(Model11, lty=2)
abline(Model22)
```
